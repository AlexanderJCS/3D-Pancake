...
def plot_times_vs_voxels(name, times, num_voxels) -> None:
    ...
    ax.text(0.05, 0.925, f'r = {r_value:.2f}', transform=ax.transAxes, fontsize=18, fontname="Calibri")
    ax.text(0.05, 0.875, f'P = {p_value:.4f}', transform=ax.transAxes, fontsize=18, fontname="Calibri")
    ax.text(0.05, 0.825, f'y = {slope:.6f}x + {intercept:.2f}', transform=ax.transAxes, fontsize=18, fontname="Calibri")

    ax.set_xlabel("Number of Voxels", fontname="Calibri", fontsize=18, fontweight="bold")
    ax.set_ylabel("Time (s)", fontname="Calibri", fontsize=18, fontweight="bold")
    ax.set_title("Runtime vs Number of Voxels", fontname="Calibri", fontsize=22, fontweight="bold", pad=10)

    for label in ax.get_yticklabels() + ax.get_xticklabels():
        label.set_fontname("Calibri")
        label.set_fontsize(18)

    plt.legend(prop={"family": "Calibri", "size": 16})
    plt.show()
