...
def geom_center(dist_map: np.array, scale: meta.Scale) -> np.ndarray:
    """
    Finds the geometric center of the PSD using the geometric center of the top 20% of the PSD distance map
    ...
    # Find the top 20% of the PSD
    top_20 = np.percentile(dist_map, 80)

    # Find the indices of the top 20% of the PSD
    top_20_indices = np.where(dist_map >= top_20)

    # Find the geometric center of the top 20% of the PSD
    center = np.mean(top_20_indices, axis=1)
